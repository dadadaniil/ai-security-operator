...
def get_summary(source_code, session_id, model):
    headers = {
    ...
    try:
        response = requests.post("http://localhost:8000/analyzer/summary", headers=headers, json=data)
...
def get_unit_tests(source_code, session_id, model, language, framework):
    headers = {
        'accept': 'application/json',
        'Content-Type': 'application/json'
    }
    ...
    try:
        response = requests.post("http://localhost:8000/testwriter/unit", headers=headers, json=data)
        if response.status_code == 200:
            return response.json()
        else:
            st.error(f"API request failed with status code {response.status_code}: {response.text}")
